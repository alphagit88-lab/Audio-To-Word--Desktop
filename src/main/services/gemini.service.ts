...
export class GeminiTranscriptionService {
  ...
  public async transcribeAudioFiles(filePaths: string[], apiKey?: string, onProgress?: (msg: string) => void, promptOptions?: ConversionPromptOptions): Promise<string> {
    ...
    const baseRule = 'Act as a professional multi language transcriptionist with 25 years experience. Transcribe with these guidelines: 1. Output spoken words WITH TIMESTAMPS at the beginning of each paragraph or speaker change. The timestamp format must be exactly [HH:MM:SS.mmm] (hours:minutes:seconds.milliseconds) relative to the start of the audio file. Example: [00:00:00.000]. 2. IF MULTIPLE people are speaking, you MUST identify and label each speaker (e.g., "[00:00:00.000] Speaker 1:" Do not change this label format. Do not add extra words after the speaker label) and insert a line break for every speaker change. 3. IF it is a SINGLE SPEAKER, no label needed, but break the text into logical paragraphs for readability, starting each paragraph with a timestamp (e.g., "[00:00:00.000]"). Do not output one massive block of text. Review twice after transcribing. If the audio starts mid-sentence, do your best to pick up the context and transcribe whatever you hear.';
